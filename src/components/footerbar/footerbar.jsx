        ...
        <If condition={MODE_SNAPPING.includes(mode)}>
          ...
          <div style={leftTextStyle}>
            ...
            />
             <FooterToggleButton
              state={this.state}
              toggleOn={() => { SharedStyle.COLORS.white='#FFF';}}
              toggleOff={() => { SharedStyle.COLORS.white='#000'; }}
              text="Mode Snap"
              toggleState={globalState.snapMask.get(SNAP_GUIDE)}
              title={translator.t('Mode Snap')}
            />
          </div>
        </If>
...

        {
          this.props.softwareSignature ?
            <div
              ...
              title={this.props.softwareSignature + (this.props.softwareSignature.includes('ArchEasy') ? '' : ` using ArchEasy ${VERSION}`)}
            >
              {this.props.softwareSignature}
            </div>
            : null
